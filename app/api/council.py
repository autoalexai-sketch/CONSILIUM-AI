"""
app/api/council.py -- Board of Directors deliberation.

run_council_deliberation() accepts optional on_phase callback.
When provided -- streams updates via WebSocket in real time.
"""
...
def _build_free_prompt(role: str, query: str, lang: str, context: str = "") -> str:
    ...
    lang_instr = f"Reply in {lang_map.get(lang, 'Russian')}. Be concise."
    ctx = f"\n\nContext:\n{context[:400]}" if context else ""
    templates = {
        "scout":     f"{lang_instr} List 3-5 key facts on: {query}",
        "analyst":   f"{lang_instr} Brief analysis (2-3 conclusions): {query}{ctx}",
        "architect": f"{lang_instr} Suggest 2-3 concrete steps: {query}{ctx}",
        "devil":     f"{lang_instr} Name 2-3 main risks or weak points: {query}{ctx}",
        "chairman":  f"{lang_instr} Give a clear comprehensive answer: {query}{ctx}",
        "operator":  f"{lang_instr} Compose a step-by-step action plan: {query}{ctx}",
    }
    return templates.get(role, f"{lang_instr} {query}")


async def _emit(on_phase: Optional[Callable], msg: dict) -> None:
    """Safe on_phase callback invocation."""
    if on_phase is None:
        return
    try:
        await on_phase(msg)
...
def _is_simple_query(query: str) -> bool:
    """True if simple factual question that does not need full council."""
    q = query.strip().lower()
    words = q.split()
    if len(words) <= 5:
        return True
    simple = [
        'weather', 'temperature', 'exchange rate', 'how much',
        'what weather', 'what today', 'what is today', 'what is',
        'who is', 'when opened', 'when does it open',
        'hello', 'hi', 'thanks', 'thank',
        'capital', 'population', 'area', 'length', 'height',
    ]
...
async def run_council_deliberation(
    ...
    if _is_simple_query(query):
        ...
        await _emit(on_phase, {"type": "phase_start", "phase": "chairman",
                               "text": "Quick answer..."})
        lang_map = {"ru": "Russian", "uk": "Ukrainian",
                   "ua": "Ukrainian", "pl": "Polish", "en": "English"}
        detected_lang = "ru"
        if any(w in query.lower() for w in ["the ", "what ", "how ", "when ", "where "]):
            detected_lang = "en"
        elif any(w in query.lower() for w in ["jak ", "czy ", "co ", "gdzie ", "kilka"]):
            detected_lang = "pl"
        elif any(ord(c) in range(0x400, 0x450) for c in query[:20]):
            detected_lang = "ru"
        lang_name = lang_map.get(detected_lang, "Russian")
        prompt = f"Reply in {lang_name}. Be accurate and helpful.\n\n{query}"
        res = await fallback_manager._call_groq(prompt)
        answer = res.get("content", "") if res.get("success") else "[Failed to get response]"
    ...
    await _emit(on_phase, {
        ...
        "text": "Discovery: analyzing cognitive complexity..."
    })
    profile = await classifier.analyze(query)
    await save_classification_log(query, profile)

    # ── Context Gateway: inject user principles + past decisions ─────────
    ...
    if not is_free and len(results) >= 2:
        await _emit(on_phase, {"type": "phase_start", "phase": "synthesizer",
                               "text": "Quality Gate: checking consistency..."})
    ...
    if chairman_result and not chairman_result.get("error") and chairman_result.get("content"):
        ...
        if syn_result and syn_result.get("analysis"):
            ...
            if coherence < 50:
                logger.warning(f"⚠️ VETO activated (coherence={coherence}%)")
                final_response = (
                    f"⚠️ [WARNING: Quality gate detected risks]\n\n"
                    f"{final_response}\n\n"
                    f"**VERIFIER ANALYSIS:** "
                    f"{syn_result['analysis'].get('meta_recommendation', 'Contradictions detected.')}"
                )
    ...
    if not final_response:
        provider_errors = errors or ["All AI providers temporarily unavailable"]
        logger.error(f"❌ All providers unavailable: {provider_errors}")
        final_response = (
            f"System temporarily unable to respond.\n\n"
            f"{''.join(f'• {e}' + chr(10) for e in provider_errors)}\n"
            f"Check API keys in .env or start Ollama."
        )
    ...
    for phase, res in results.items():
        if res and isinstance(res, dict):
            deliberation[phase] = {
                ...
                "preview": str(res.get("content", ""))[:300] + "..." if res.get("content") else "No data",
    ...
    return {
        ...
        "council": {
            ...
            "directors": {k: {"model": getattr(v, "model", str(v)),
                              "description": getattr(v, "description", "Council member")}
